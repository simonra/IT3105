...
public class KnapsackFileReader {
	...
	public static Double[][] standardKnapsack(String ksText, Random r){
		...
		for (int i = 0; i < itemLines.length; i++) {
			solutionSpace[i][0] = Double.parseDouble(itemLines[i].split(",")[0]);
			solutionSpace[i][1] = Double.parseDouble(itemLines[i].split(",")[1]);
			solutionSpace[i][2] = 1 + Constants.KNAPSACKMAXVOLUMEPERITEM * r.nextDouble();
		}
